...
for (let i = 0; i < draggableElements.length; i++) {
  ...
  draggableElements[i].addEventListener('click', (e) => {
    addEventListener('keydown', (e) =>{
      // console.log(e.key);
      // console.log(draggableElements[i].style.height);
      // console.log(draggableElements[i].style.width);

      if(e.key <= 9 && e.key >= 1)
      {
        let nstyleY =  ( parseInt(e.key, 10) * parseInt(draggableElements[i].style.height, 10));
        let nstyleX =  ( parseInt(e.key, 10) * parseInt(draggableElements[i].style.width, 10));
        // console.log( "Новый Y "+ nstyleY);
        // console.log( "Новый X "+nstyleX);
        draggableElements[i].style.height = nstyleY + 'px';
        draggableElements[i].style.width = nstyleX + 'px';
      }
      else
      {
        draggableElements[i].style.height = parseInt(draggableElements[i].style.height, 10) * 0.5 + 'px';
        draggableElements[i].style.width = parseInt(draggableElements[i].style.width, 10) * 0.5 + 'px';
      }
    })
  })
